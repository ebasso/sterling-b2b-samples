        ...
        for line in file:
            if "PNUM=" in line:
                ...
                if pnum not in pnum_dict:
                    pnum_dict[pnum] = {"RECI_SSTR": False, "RECI_CTRC": False, "RECI_SEND": False, "STAR": star}

                # define and update RECID
                if "SSTR" in line:
                    pnum_dict[pnum]["RECI_SSTR"] = True
                if "CTRC" in line:
                    pnum_dict[pnum]["RECI_CTRC"] = True
                if "SEND" in line:
                    pnum_dict[pnum]["RECI_SEND"] = True
...
def process_directory(directory_path):
    ...
    for pnum, flags in pnum_dict.items():
       if not pnum_dict[pnum]["RECI_CTRC"]:
           print(f"PNUM={pnum}, RECI_SSTR={flags['RECI_SSTR']}, RECI_CTRC={flags['RECI_CTRC']}, RECI_SEND={flags['RECI_SEND']}, STAR={flags['STAR']}")
